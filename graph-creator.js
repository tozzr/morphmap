var setupGraphEditor = function(d3, saveAs, Blob, graphData) {
  "use strict";
  ...
  var svg = d3
    ...
    .attr("width", width)
    .attr("height", height);
  var graph = new GraphCreator(svg, graphData.nodes, graphData.edges);
  graph.setIdCt(2);
  graph.updateGraph();
};
/*
document.onload = setupGraphEditor(window.d3, window.saveAs, window.Blob, {
  nodes: [],
  edges: []
});
*/

fetch("mydag.json") // Call the fetch function passing the url of the API as a parameter
  .then(function(res) {
    return res.json();
  })
  .then(function(data) {
    data.edges.forEach(function(e, i) {
      data.edges[i] = {
        source: data.nodes.filter(function(n) {
          return n.id == e.source;
        })[0],
        target: data.nodes.filter(function(n) {
          return n.id == e.target;
        })[0]
      };
    });
    setupGraphEditor(window.d3, window.saveAs, window.Blob, data);
  })
